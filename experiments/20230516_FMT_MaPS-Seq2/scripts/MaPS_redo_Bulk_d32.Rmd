...
zotu <- read_csv("../processed_data/d32/zotu.csv") %>% mutate(seqid = gsub(.$seqid, replacement = '_', pattern = '-'))
tax <- read_csv("../ref/d32/tax.csv")
meta <- read_csv("../ref/d32/metadata.csv")
...
vendor <- zotu %>% 
  select(OTU, sample, log10relabd) %>% reshape2::dcast(OTU~sample)
...
vendor.long <- left_join(vendor.long, gtable) %>% #add family data
    left_join(., (select(zotu, mouse, mouse_type, sample, day) %>% distinct()), by = c('variable'='sample')) %>%
    separate(variable, into = c('tx_group', 'mouse_num'))
...
#vendor.long$day <- factor(vendor.long$day,      # Reordering group factor levels
#                         levels = c(0,32))
# Heatmap
heatmap.plot <- ggplot(data = vendor.long, aes(x = mouse, y = OTU)) +
...
ggsave("../figs/day32.png",  width = 10, height = 5)
ggsave("../figs/day32.pdf",  width = 10, height = 5)
