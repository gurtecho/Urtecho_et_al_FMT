...
#conda install -c conda-forge r-base   #can't install 
#Load software assuming ou use Tum's AMI
conda install -c conda-forge -c bioconda ultraplex seqkit vsearch

#Define # of threads used 
THREADS=16
...
foo () {
    ...
    ./bin/usearch10 -fastq_filter "${FILENAME}" -fastq_maxee 1.0 -fastq_minlen 150 -relabel @ -fastqout "${OUTFILE}" -threads $30
}
...
find ./fastq/*_filtered.fastq.gz -type f | while read FILENAME ; do
	...
	ultraplex -i "${FILENAME}" -b ./bin/BarcodeCSV7leadingfull.csv -l 86 -inm -t $THREADS -dbr -d fastq/first7bp -q 20
	ultraplex -i "${FILENAME}" -b ./bin/BarcodeCSV8leadingfull.csv -l 86 -inm -t $THREADS -dbr -d fastq/first8bp -q 20
	ultraplex -i "${FILENAME}" -b ./bin/BarcodeCSV9leadingfull.csv -l 86 -inm -t $THREADS -dbr -d fastq/first9bp -q 20
	...
	seqkit amplicon  ./fastq/AllLengths.fastq.gz -F YCAGCMGCCGCGGTAA -r 1:240 -f  -j $THREADS -o "${OUTFILE}";
...
./bin/usearch10 -fastx_uniques otu/filt_all_double.fq -sizeout -relabel Uniq -fastaout otu/uniques.fa -threads $THREADS
...
vsearch --usearch_global ./otu/filt_all_x.fa --db ./otu/zotus.udb --id 0.9 --otutabout ./ProcessedResults/otu_frequency_table.tsv --biomout ./ProcessedResults/otu_frequency_table.biom --threads 16
