...
library(tidyverse)
theme_set(theme_bw())
setwd("~/Dropbox/fmt/experiments/20230516_FMT_MaPS-Seq2/20230823_MaPSFMT_redo_ProcessingScripts/ProcessedResults/")
#################
melteddf2 <- fread("../ProcessedResults/melted_otu_table.csv") %>% ungroup()
...
#If no Bulk, just skip here:
settocorrect <- melteddf2
unique(melteddf2$Particle)
...
#settocorrect[,ReadNum := str_extract(Barcode1,"_[0-9]+")] #idk what this is!
...
CorrectedDataframe <- finaldf[, sum(Count), by = .(CorrectedParticleID,OTU,Sample)] #if no bulk
...
fwrite(CorrectedDataframe,"../ProcessedResults/CorrectedDataFrame.csv")
