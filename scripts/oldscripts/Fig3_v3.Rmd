...
3B number of species lost over time:
...
change_df <- data.frame(day = as.character(1:30))

for (i in unique(zotu$day)) {
  ...
  gain_data <- data.frame(num_added = nrow(gained), day = day_test )
  ...
  lost <- test_ds %>% as.data.frame() %>%
      ...
      loss_data <- data.frame(num_lost = nrow(lost), day = day_test )

  change_df<- left_join(change_df, left_join(gain_data, loss_data))
}
