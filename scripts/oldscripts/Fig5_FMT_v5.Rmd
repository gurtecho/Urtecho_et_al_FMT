...
How many interactions per mouse group between how many species CALCULATED WITH INTERACTION BARS
```{}

rbind(select(PrevalentSets, MouseGroup, OTU1, OTU2, Significant, Zscore)%>% filter(Significant == T) %>% mutate(interaction_type = ifelse(Zscore > 0, 'Positive', 'Negative')) , 
  (select(PrevalentSets, MouseGroup, 'OTU1'='OTU2', 'OTU2'='OTU1', Significant, Zscore) %>% 
     filter(Significant == T) %>% mutate(interaction_type = ifelse(Zscore > 0, 'Positive', 'Negative')))) %>%
  select(-Zscore) %>% distinct() %>% group_by(MouseGroup) %>% 
  mutate(num_interactions = n()/2) %>% ungroup %>%
  select(MouseGroup, num_interactions) %>% distinct()

```


#5c? Make network diagram of mean interactions, show present nodes arranged in auto
```{r}
...
Instances <- PrevalentSets %>% select(MouseGroup, OTU1, Cluster, InstancesOTU1) %>% distinct() %>%
  ungroup() %>% group_by(MouseGroup, OTU1) %>% 
  mutate(total_instances_OTU1 = sum(InstancesOTU1)) %>%
  ungroup() %>% select(OTU1, total_instances_OTU1,MouseGroup) %>% distinct() %>%
...
cluster_network <- mean_interactions %>% 
...
temp <- cluster_network
colnames(temp) = gsub("OTU1", "OTUx", colnames(temp))
colnames(temp) = gsub("OTU2", "OTU1", colnames(temp))
colnames(temp) = gsub("OTUx", "OTU2", colnames(temp))

cluster_network <- rbind(cluster_network, temp)
...
tbl_nodes <- cluster_network %>% filter(MouseGroup == 'Pre FMT') %>% 
  left_join(., Instances, by = c("OTU1", "MouseGroup")) %>%
  mutate(OTU1 = gsub('Otu', '', .$OTU1)) %>% select(OTU1, total_instances_OTU1, Order_OTU1)%>% 
  distinct() %>%
  mutate(total_instances = as.numeric(total_instances_OTU1)) %>% select(-total_instances_OTU1)

tbl_nodes[is.na(tbl_nodes)] <- 0
...
#who has most interactions?
edge_count_pre <- tbl_edges %>% group_by(OTU1) %>% mutate(num_edges = n()) %>%  select(OTU1, num_edges) %>% distinct() %>% ungroup() %>% left_join(tbl_nodes)
...
nodes <- ggraph(graph_nodes, layout = 'auto') +
   geom_edge_link(aes(edge_width = abs(average_corr)*.01, 
    ...
    scale_edge_color_manual(values = c('navyblue','firebrick')) +
      scale_edge_width_continuous(range = c(0,5)) +
        geom_node_point(aes(size = total_instances*.4),color = 'black',) +
...
#who has most interactions? ALL BACTEROIDIA
edge_count_post <- tbl_edges %>% group_by(OTU1) %>% mutate(num_edges = n()) %>%  select(OTU1, num_edges) %>% distinct() %>% ungroup() %>% left_join(tbl_nodes)
...
```{r}
rbind(edge_count_pre %>% mutate(MouseGroup = 'Pre FMT'),
      edge_count_post %>% mutate(MouseGroup = 'Post FMT')) %>%
...
PrevalentSets %>% filter(Significant == T) %>% 
  ...
  ggplot(., aes(x = factor(MouseGroup, levels = c('Pre FMT', 'Donor', 'Post FMT')),
                fill = OTU1Type)) + 
...
rbind((temp %>% select(MouseGroup, OTU2=OTU1, OTU1=OTU2, OTU2Type=OTU1Type, OTU1Type=OTU2Type, Significant)), temp) %>% distinct() %>%
  ...
  group_by(MouseGroup, OTU1Type, OTU2Type) %>% mutate(prcnt_interactions = signif(n()/grp_interactions, 3)*100) %>% 
  ungroup() %>% select(MouseGroup, OTU1Type, OTU2Type, prcnt_interactions) %>% 
  distinct() %>% 
  ggplot(., aes(OTU1Type, OTU2Type, fill = (prcnt_interactions))) + geom_tile() +
  geom_text(aes(label = prcnt_interactions), color = 'white') +
...
temp <- PrevalentSets %>% 
  ...
  select(MouseGroup, OTU1Type, OTU2Type,OTU1, OTU2,Order_OTU1, Order_OTU2, interaction_type) %>%
...
temp$interaction <- factor(temp$interaction, levels=c("donor-donor", "recipient-recipient", "donor-recipient", "donor-shared", "recipient-shared", "shared-shared")) 
...
temp %>% filter(interaction != 'donor-donor' & interaction != 'donor-recipient' & interaction != 'recipient-recipient') %>% 
  ...
  theme_clean() + 
  facet_wrap(~factor(MouseGroup, levels = c('Pre FMT', 'Post FMT', 'Donor')), nrow = 1, strip.position = 'top') +
...
Interesting, preFMT - specific communities don't interact much! Universal microbes do interact in all communities. Donor microbes also are able to interact with shared bugs, who are these interactors?


```{}
#in pre FMT, who are shared interactors?
#This time, need to double temp
temp_fam <- rbind(select(temp, MouseGroup, OTU1, OTU2, Family_OTU1, Family_OTU2, interaction_type, interaction), 
  (select(temp, MouseGroup, 'OTU1'='OTU2', 'OTU2'='OTU1', 'Family_OTU2'='Family_OTU1', 'Family_OTU1'='Family_OTU2', interaction_type,interaction))) %>%  
  group_by(MouseGroup, Family_OTU1, Family_OTU2, interaction_type ,interaction) %>% mutate(num_interactors = n()) %>% ungroup() %>% select(-OTU1, -OTU2) %>% mutate(num_interactors = as.numeric(num_interactors)/2) %>% distinct()

grid <- rbind(data.frame(Family_OTU1=temp$Family_OTU1), data.frame(Family_OTU1=temp$Family_OTU1)) %>% 
  distinct() %>% 
  mutate(merge = 'merge')

grid <- left_join(grid, grid, by = 'merge') %>% mutate(num_interactors=0) %>% rename('Family_OTU1'='Family_OTU1.x', 'Family_OTU2'='Family_OTU1.y') %>% semi_join(., temp_fam %>%  filter(interaction == 'donor-shared' | interaction == 'recipient-shared'), by = 'Family_OTU1') %>% semi_join(., temp_fam %>%  filter(interaction == 'donor-shared' | interaction == 'recipient-shared'), by = 'Family_OTU2') 



temp_fam %>% # filter(interaction == 'donor-shared' | interaction == 'recipient-shared') %>% group_by(Family_OTU1, Family_OTU2, MouseGroup) %>% mutate(sum_interactions = sum(num_interactors)) %>% select(-num_interactors, - interaction) %>% distinct() %>% filter(Family_OTU2 != 'Peptococcaceae') %>%
  filter(Family_OTU1 != 'Peptococcaceae') %>%
   ggplot(., aes(x=Family_OTU1, y = Family_OTU2)) +
    geom_tile(data = grid, aes(x=Family_OTU1, y = Family_OTU2), color = 'black', fill = NA) +
  geom_tile(aes(fill = sum_interactions), color = 'black') + facet_grid(~MouseGroup) +
  labs(fill = '# of interactions') +
  scale_fill_gradientn(colors = pal) +
  theme_cowplot() +
  labs(y = 'Family', x = 'Family', title = 'Interactions with Shared Taxa') +
...
ggsave("../figs/s5/S5X_FamilyInteractions.png", width = 8, height = 5.5)

```


Now let's do order split by interaction type
```{r}

normalize_PostFMT <- (temp %>% filter(MouseGroup == 'Post FMT') %>% 
                        select(OTU=OTU1, Order_OTU=Order_OTU1)) %>% 
  rbind((temp %>% filter(MouseGroup == 'Post FMT') %>% 
           select(OTU1=OTU2, Order_OTU=Order_OTU2))) %>% distinct() %>% 
  mutate(merge = 'xyz')%>% full_join(.,., by = 'merge') %>%
  select(Order_OTU1=Order_OTU.x, Order_OTU2=Order_OTU.y) %>% group_by(Order_OTU1, Order_OTU2) %>%
  mutate(num_pairs = n()) %>% distinct() %>% ungroup()

normalize_PreFMT <- (temp %>% filter(MouseGroup == 'Pre FMT') %>% 
                        select(OTU=OTU1, Order_OTU=Order_OTU1)) %>% 
  rbind((temp %>% filter(MouseGroup == 'Pre FMT') %>% 
           select(OTU1=OTU2, Order_OTU=Order_OTU2))) %>% distinct() %>% mutate(merge = 'xyz') %>%
  full_join(.,., by = 'merge') %>%
  select(Order_OTU1=Order_OTU.x, Order_OTU2=Order_OTU.y) %>% group_by(Order_OTU1, Order_OTU2) %>%
  mutate(num_pairs = n()) %>% distinct() %>% ungroup()


temp_2 <- rbind(select(temp, MouseGroup, OTU1, OTU2, Order_OTU1, Order_OTU2, interaction_type, interaction), 
  (select(temp, MouseGroup, 'OTU1'='OTU2', 'OTU2'='OTU1', 'Order_OTU2'='Order_OTU1', 'Order_OTU1'='Order_OTU2', interaction_type,interaction))) %>%  
  group_by(MouseGroup, Order_OTU1, Order_OTU2, interaction_type ,interaction) %>% 
...
grid <- rbind(data.frame(Order_OTU1=temp$Order_OTU1), data.frame(Order_OTU1=temp$Order_OTU1)) %>% 
  distinct() %>% 
  mutate(merge = 'merge')

grid <- left_join(grid, grid, by = 'merge') %>% 
  mutate(num_interactors=0) %>% 
  rename('Order_OTU1'='Order_OTU1.x', 'Order_OTU2'='Order_OTU1.y') %>% 
  semi_join(., temp_2, by = 'Order_OTU1')
  #semi_join(., temp_2 %>%  filter(interaction == 'donor-shared' | interaction == 'recipient-shared'), by = 'Order_OTU1') %>% semi_join(., temp_2 %>%  filter(interaction == 'donor-shared' | interaction == 'recipient-shared'), by = 'Order_OTU1') 

temp_2 %>% filter(interaction_type == 'Negative') %>%
 distinct() %>% filter(MouseGroup != 'Donor') %>%
  #filter(interaction == 'donor-shared' | interaction == 'recipient-shared') %>%
  group_by(Order_OTU1, Order_OTU2, MouseGroup) %>% mutate(sum_interactions = sum(num_interactors)) %>% select(-num_interactors, - interaction) %>% distinct() %>%
   ggplot(., aes(x=Order_OTU1, y = Order_OTU2)) +
    geom_tile(data = grid, aes(x=Order_OTU1, y = Order_OTU2), color = 'black', fill = NA) +
  geom_tile(aes(fill = sum_interactions), color = 'black') +
    scale_fill_gradient(low='lightblue', high='navyblue') +
facet_grid(~factor(MouseGroup, levels = c('Pre FMT', 'Post FMT'))) +
  labs(fill = '# of - interactions') +
  theme_cowplot() +     
  geom_text(aes(label=sum_interactions), size = 2.5, color = 'white') +
  labs(y = 'Order', x = 'Order', title = 'Interactions with Shared Taxa') +
    theme(axis.text.x = element_text(size=9,color = 'black', 
                                     angle = 45, hjust = 1), 
        axis.text.y = element_text(size=9, color = 'black'),
        axis.title.x.bottom = element_text(size = 14,color = 'black'),
...
ggsave("../figs/fig5/5D_OrderInteractions_neg.pdf", width = 5, height = 4)

temp_2 %>% filter(interaction_type == 'Positive') %>%
 distinct() %>% filter(MouseGroup != 'Donor') %>%
  #filter(interaction == 'donor-shared' | interaction == 'recipient-shared') %>%
  group_by(Order_OTU1, Order_OTU2, MouseGroup) %>% 
  mutate(sum_interactions = sum(num_interactors)) %>% select(-num_interactors, - interaction) %>% distinct() %>%
   ggplot(., aes(x=Order_OTU1, y = Order_OTU2)) +
    geom_tile(data = grid, aes(x=Order_OTU1, y = Order_OTU2), color = 'black', fill = NA) +
  geom_tile(aes(fill = sum_interactions), color = 'black') + facet_grid(~factor(MouseGroup, levels = c('Pre FMT', 'Post FMT'))) +
  labs(fill = '# of + interactions') +
...
ggsave("../figs/fig5/5D_OrderInteractions_pos.pdf", width =5, height = 4)
...
temp_2 %>% filter(interaction_type == 'Positive') %>%
 distinct() %>% 
  #filter(interaction == 'donor-shared' | interaction == 'recipient-shared') %>%
  group_by(Order_OTU1, Order_OTU2, MouseGroup) %>% mutate(sum_interactions = sum(num_interactors)) %>% select(-num_interactors, - interaction) %>% distinct() %>%
    ggplot(.,aes(Order_OTU1, 1, fill=Order_OTU1)) +
  geom_tile() +
...
ggsave("../figs/fig5/5D_OrderInteractions_labels.pdf", width = 1, height = .3)
...
```{r}
#What happens to donor microbes?
engrafted_donors <- 
  rbind(PrevalentSets %>% #need to flip Prevalent sets so interactions go both ways correct
  filter(MouseGroup == 'Post FMT' & zpadjusted < .05 & OTU1Type != 'Recipient Taxa') %>% select(OTU1, OTU2),
    rbind(PrevalentSets %>% #need to flip Prevalent sets so interactions go both ways correct
  filter(MouseGroup == 'Post FMT' & zpadjusted < .05 & OTU2Type != 'Recipient Taxa') %>% select(OTU1=OTU2, OTU2=OTU1))) %>% distinct() %>% filter(OTU1!=OTU2)%>%
  ...
  distinct() 
engrafted_donors$num_interactions[is.na(engrafted_donors$num_interactions)] <- 0 


engrafted_donors %>% ggplot(., aes(factor(emergence, levels = c('depleted', 'unchanged', 'enriched')), 
                                   num_interactions)) + 
  geom_boxplot(notchwidth = .2, outlier.shape = NA, width = .4, fill = 'lightblue') +
  geom_jitter() + 
  ...
  labs(x = 'Donor Strain FMT Outcome', y = '# of Interactions', fill =NA) + 
...
engrafted_rec <- 
  rbind(PrevalentSets %>% #need to flip Prevalent sets so interactions go both ways correct
  filter(MouseGroup == 'Pre FMT' & zpadjusted < .05 & OTU1Type != 'Donor Taxa') %>% select(OTU1, OTU2),
    rbind(PrevalentSets %>% #need to flip Prevalent sets so interactions go both ways correct
  filter(MouseGroup == 'Pre FMT' & zpadjusted < .05 & OTU2Type != 'Donor Taxa') %>% select(OTU1=OTU2, OTU2=OTU1))) %>% distinct() %>% filter(OTU1!=OTU2)%>%
  group_by(OTU1) %>% mutate(num_interactions = n()) %>% 
  select(OTU1, num_interactions) %>% left_join(filter(otumapping, OTUType!='Donor Taxa')%>% select(OTU1=OTU, emergence, OTUType), .) %>%
  distinct()
...
How dense are interaction networks in Envigo vs 
```{r}
