...
coltype <- left_join(jax.cohoused.long, read.csv("../tables/colonizer_order.txt")) %>% replace(is.na(.), 'Non') 
coltype$OTU <- factor(x = coltype$OTU,
                               levels = jax.cohoused.scaled$OTU[jax.cohoused.order], 
                               ordered = TRUE)
coltype <- ggplot(coltype, aes(x = '', y = OTU)) +
    geom_tile(aes(fill = as.factor(colonizer_type))) +
    scale_fill_manual(values = c('#fbb03b', '#ff0000','white')) +
...
res_d32d5 <- results(dds, contrast=c("day","32","5"), 
                     pAdjustMethod = 'BH')
...
#Donor
res_d32d5 <- res_d32d5 %>% as.data.frame() %>%
                  mutate(threshold = padj < 0.05 & abs(log2FoldChange) >= 1) %>% rownames_to_column('OTU') %>% left_join(., tax)
...
rbind(res_d5d0 %>% filter(threshold == T & log2FoldChange > 0) %>% mutate(colonizer_type = 0),
      res_d32d5 %>% filter(threshold == T & log2FoldChange > 0) %>% mutate(colonizer_type = 1)) %>% 
      select(OTU, colonizer_type, order, family) %>% 
      group_by(OTU) %>% 
      mutate(colonizer_type = sum(colonizer_type)) %>% ungroup %>% 
      mutate(colonizer_type = ifelse(colonizer_type == 1, 'Late', 'Early')) %>% distinct %>% 
  write.csv("../tables/colonizer_order.txt", quote = F, row.names = F)
...
Fig 3d - How does absolute abundance of envigo/jax microbes change over time?
