...
coltype <- full_join((jax.cohoused.long %>% filter(variable == '0') %>% mutate(d0_abd = 10^value) %>% select(OTU, d0_abd)),
  ...
  mutate(colonizer_type = ifelse(d32_abd/d0_abd > 10, 2, 0),
    ...
    ggplot(., aes(x = '', y = OTU)) +
    geom_tile(aes(fill = as.factor(colonizer_type)), color = 'black') +
...
cowplot::plot_grid(env, heatmap.plot, fam, coltype, ncol =4, rel_widths = c(.1,.6,.2, .2))
...
Fig 3C - How does absolute abundance of envigo/jax microbes change over time?
...
3C number of species over time
...
#write out table
write.table(colonizers_delta, "../tables/colonizer_order_3B.txt", quote = F, row.names = F)
...
Supplementary FIgure X - PCA before/after cohousing
```{r}
...
endpoints <- norm_zotu %>%
  ...
  #       mutate(mean_log10reladb = mean(log10abs_rel_abundance)) %>%
  #     filter(mean_log10reladb > .003) %>% 
  ungroup() %>%
 # mutate(mean_log10reladb = mean(log10abs_rel_abundance)) %>%
  filter(day == 0 | day == 30) %>% 
...
tSNE_header[tSNE_header == "Jax Cohoused_0"] <- "Jax Day 0"
tSNE_header[tSNE_header == "Jax Cohoused_30"] <- "Jax Day 30"
...
Supplementary Fig X - Shannon Diversity of 16S seqs before/after cohousing
...
sd_df<- 10^tSNE_input %>% t() %>% vegan::diversity()

sd_lab <- tSNE_header %>% t()

shannonD <- sd_df %>% as.data.frame() %>% cbind(., sd_lab) %>% select(shannD = '.', grp_label)

pval <- wilcox.test(filter(shannonD, grp_label == 'Jax Control\n Day 30')$shannD,
                    filter(shannonD, grp_label == 'Jax Cohoused\n Day 30')$shannD, 
                    alternative = 'less')$p.value

#all side by side
a<- ggplot(shannonD, aes(grp_label, shannD, fill = grp_label)) +
...
b <- ggplot(shannonD, aes(grp_label, shannD, fill = grp_label)) + 
  geom_boxplot(outlier.shape = NA) + geom_jitter(size = 2) +
  scale_x_discrete(limits = c('Env Day 30', 'Jax2Env Day 30',
                              'Jax Day 30', 'Env2Jax Day 30')) +
  labs(y = '', x = '', title = 'Day 30') +
  scale_fill_manual(values = c('lightblue', 'blue', 'green', 'darkgreen', 'yellow', 'gold', 'orchid', 'purple')) +
  annotate('text', x = 'Jax Control\n Day 30', y = 3.75, hjust = -.2, label = signif(pval, 3)) +
  geom_segment(x = 'Jax Day 30', xend = 'Env2Jax Day 30', y = 3.7, yend =3.7) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1), 
axis.title.x.bottom = element_text(size = 12),
...
ggsave("../figs/s3/S3B_cohouse_ShannonDiversity.png", width = 10, height = 3.5)
ggsave("../figs/s3/S3B_cohouse_ShannonDiversity.pdf", width = 10, height = 3.5)
...
Supplementary Figure X - Temporal dynamics of controls
```{r}
jax.cohoused.long <- norm_zotu %>% mutate(OTU = as.factor(OTU)) %>% 
    filter(tx_group != 'Jax Cohoused' & tx_group != 'Envigo Cohoused') %>%
...
ggsave("../figs/s3/S3C_control_relabd.png", width = 10, height = 3.5)
ggsave("../figs/s3/S3C_control_relabd.pdf", width = 10, height = 3.5)
...
ggsave("../figs/s3/S3C_fam.pdf", width = 10, height = 3.5)
...
Look at OTUs gained and Displaced when not cohoused Envigo
```{r}
...
Figure S3E - biomass of controls over time
```{r}
...
a <- ggplot(temp, aes(x=day,
                y=class_abd,
                fill=factor(class, levels = c('Donor Specific', 'Shared', 'Recipient Specific')))) +  
  geom_bar(stat="identity", position = 'stack') +
  labs(x='Day', y = 'Relative Biomass', fill = 'Microbe Class', title = 'Jackson Controls') +
  scale_fill_manual(values = c( ven[3],'gray80', ven[2])) +
  ylim(0,1.2) +
  theme_cowplot() +
    theme(plot.background = element_blank(),
...
b <- ggplot(temp, aes(x=day,
                y=class_abd,
                fill=factor(class, levels = c('Donor Specific', 'Shared', 'Recipient Specific')))) +  
  geom_bar(stat="identity", position = 'stack') +
  labs(x='Day', y = 'Relative Biomass', fill = 'Microbe Class', title = 'Envigo Controls') +
  scale_fill_manual(values = c( ven[3],'gray80', ven[2])) +
  theme_cowplot() +
  ylim(0,1.2) +
...
ggsave("../figs/s3/3F_ctrl_abundance.png",  width = 6, height = 5)
ggsave("../figs/s3/3F_ctrl_abundance.pdf",  width = 6, height = 5)
