...
Tax_Table <- data.table::fread("~/Dropbox/fmt/experiments/2022-09-13 Guillaume FMT r3/SilvaTax.csv") %>% 
  rename('Class'='Order', 'Order'='Class')
...
plotframe <- plotframe %>% 
  ...
  mutate(noFMT_score = ifelse(MouseGroup == 'Jax',log10(musgroup_proportion_cluster), NA),    
         ...
         donor_score = ifelse(MouseGroup == 'Env',log10(musgroup_proportion_cluster), NA)) %>%
...
bugColors <- data.frame('bugColors'=c(data.frame(bugColors)$bugColors[2:15], data.frame(bugColors)$bugColors[1])) %>% mutate(names = levels(as.factor(PrevalentSets$Order_OTU1))) %>% column_to_rownames('names') 

colors <- bugColors$bugColors
names(colors) = row.names(bugColors)
bugColors <- colors
...
nodes <- ggraph(graph_nodes, layout = 'auto') +
   geom_edge_link(aes(#edge_width = abs(average_corr)*.01, 
                      ...
                      edge_color = interaction), edge_width = .75) +
...
Who is interacting? S6, 5D
```{r}
interactionSET <- rbind(select(PrevalentSets, MouseGroup, OTU1, OTU2, Order_OTU1, Order_OTU2, OTU1Type, OTU2Type, Zscore, Significant) %>% mutate(orientation = 'r'), 
  (select(PrevalentSets, MouseGroup, 'OTU1'='OTU2', 'OTU2'='OTU1', 'Order_OTU2'='Order_OTU1', 'Order_OTU1'='Order_OTU2', 'OTU1Type'='OTU2Type', 'OTU2Type'='OTU1Type', Zscore, Significant)%>% mutate(orientation = 'l'))) %>% 
    distinct() %>% 
  ...
  mutate(interaction = ifelse(OTU1Type == 'Donor Taxa' & OTU2Type == 'Donor Taxa', 'donor-donor', interaction),
         ...
         interaction = ifelse(OTU1Type == 'Shared Taxa' & OTU2Type == 'Recipient Taxa', 'recipient-shared', interaction),
         interaction = ifelse(OTU1Type == 'Shared Taxa' & OTU2Type == 'Donor Taxa', 'donor-shared', interaction)) %>% na.omit() %>% 
      select(-Zscore) %>% 
      distinct() %>%
    group_by(MouseGroup, Order_OTU1, Order_OTU2, interaction_type ,interaction) %>%
  mutate(num_interactors = n()) %>% ungroup()


interactionSET$interaction <- factor(interactionSET$interaction, levels=c("donor-donor", "recipient-recipient", "donor-recipient", "donor-shared", "recipient-shared", "shared-shared")) 
  

#What types of interactions do we see
interactionSET %>% group_by(MouseGroup, interaction_type, interaction) %>% mutate(num_interactions = n()/2) %>% ungroup %>% select(MouseGroup, interaction_type, interaction, num_interactions) %>% distinct() %>%
 ggplot(., aes(x=interaction, y =num_interactions, fill = interaction_type)) + 
...
ggsave("../figs/s6/S6_ALLInteractionCounts.png", width = 3.5, height = 5)
ggsave("../figs/s6/S6_ALLInteractionCounts.pdf", width = 3.5, height = 5)

```

5D- Num interactions between groups
```{r}
interactionSET <- interactionSET %>% filter(MouseGroup != 'Donor')
grid <- rbind(data.frame(Order_OTU1=interactionSET$Order_OTU1), data.frame(Order_OTU1=interactionSET$Order_OTU1)) %>% 
  distinct() %>% 
  mutate(merge = 'merge')
...
grid <- left_join(grid, grid, by = 'merge') %>% 
  ...
  semi_join(., interactionSET, by = 'Order_OTU1')


interactionSET %>% 
  filter(interaction_type == 'Negative') %>% 
  distinct() %>% select(Order_OTU1, Order_OTU2, MouseGroup, num_interactors) %>% distinct() %>%
  group_by(Order_OTU1, Order_OTU2, MouseGroup) %>% 
  mutate(sum_interactions = sum(num_interactors)) %>% select(-num_interactors) %>% distinct() %>%   ggplot(., aes(x=Order_OTU1, y = Order_OTU2)) +
...
facet_grid(~factor(MouseGroup, levels = c('Pre FMT', 'Post FMT'))) +  
  ...
  theme_cowplot() +     
  geom_text(aes(label=sum_interactions), size = 2.5) +
  labs(y = 'Order', x = 'Order') +
...
ggsave("../figs/fig5/5D_OrderInteractions_neg.pdf", width = 6, height = 4.5)

interactionSET %>% filter(interaction_type == 'Positive') %>%
   distinct() %>% select(Order_OTU1, Order_OTU2, MouseGroup, num_interactors) %>% distinct() %>%
  group_by(Order_OTU1, Order_OTU2, MouseGroup) %>% 
...
ggsave("../figs/fig5/5D_OrderInteractions_pos.pdf", width = 6, height = 4.5)

#make axis legend
grid %>% ggplot(.,aes(Order_OTU1, 1, fill=Order_OTU1)) +
...
ggsave("../figs/fig5/5D_OrderInteractions_labels.pdf", width = 1, height = .25)
...
temp_2 <- rbind(select(interactionSET, MouseGroup, OTU1, OTU2, Order_OTU1, Order_OTU2, interaction_type, interaction), 
  (select(interactionSET, MouseGroup, 'OTU1'='OTU2', 'OTU2'='OTU1', 'Order_OTU2'='Order_OTU1', 'Order_OTU1'='Order_OTU2', interaction_type,interaction))) %>%  
  group_by(MouseGroup, Order_OTU1, Order_OTU2, interaction_type ,interaction) %>% mutate(num_interactors = n()) %>% ungroup() #%>% select(-OTU1, -OTU2) %>% mutate(num_interactors = as.numeric(num_interactors)) %>% distinct()
...
grid <- left_join(grid, grid, by = 'merge') %>% 
  mutate(num_interactors=0) %>% 
  rename('Order_OTU1'='Order_OTU1.x', 'Order_OTU2'='Order_OTU1.y') %>% semi_join(., temp_2 %>%  filter(interaction == 'donor-shared' | interaction == 'recipient-shared'), by = 'Order_OTU1') %>% semi_join(., temp_2 %>%  filter(interaction == 'donor-shared' | interaction == 'recipient-shared'), by = 'Order_OTU1') %>%
  filter(Order_OTU2 != 'Veillonellales-Selenomonadales' & Order_OTU2 != 'Synergistales') %>%
  filter(Order_OTU2 != 'Christensenellales' & Order_OTU2 != 'Bifidobacteriales' & Order_OTU2 != 'Enterobacterales')


temp_2 %>% filter(interaction_type == 'Negative') %>%  
  filter(Order_OTU2 != 'Veillonellales-Selenomonadales' & Order_OTU2 != 'Synergistales') %>%
    filter(Order_OTU2 != 'Christensenellales' & Order_OTU2 != 'Bifidobacteriales' & Order_OTU2 != 'Enterobacterales') %>%
 distinct() %>% filter(interaction == 'donor-shared' | interaction == 'recipient-shared') %>% 
...
ggsave("../figs/fig5/5D_OrderInteractions_neg.pdf", width = 5, height = 3.5)

temp_2 %>% filter(interaction_type == 'Positive') %>%
  filter(Order_OTU1 != 'Peptococcales' & Order_OTU2 != 'Peptococcales') %>%
 distinct() %>% filter(interaction == 'donor-shared' | interaction == 'recipient-shared') %>% group_by(Order_OTU1, Order_OTU2, MouseGroup) %>% mutate(sum_interactions = sum(num_interactors)) %>% select(-num_interactors, - interaction) %>% distinct() %>%
   ggplot(., aes(x=Order_OTU1, y = Order_OTU2)) +
    geom_tile(data = grid, aes(x=Order_OTU1, y = Order_OTU2), color = 'black', fill = NA) +
...
ggsave("../figs/fig5/5D_OrderInteractions_pos.pdf", width = 5, height = 3.5)
...
#Are engrafting microbes growing in MM?
