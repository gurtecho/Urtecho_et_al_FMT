...
3D What is the abundance of Donor vs Recipient vs shared taxa across the gut
...
S5A - How do gut compartments localize now? PCA euclidean distance by absolute # of reads
```{r}
...
S5C - Are late/early colonizers colonizing specific areas?



12/17/23 - Needed to fix this since it didn't properly normalize for proportion of biomass. before it jsut had absolute reads at each spot (which were normalized across samples). now we normaize reads to total in gut compartment so allll good.
